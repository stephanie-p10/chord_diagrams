...
class RowColumnSeparationStrategy(DisjointUnionStrategy[Tiling, GriddedChord]):
    """
    ...
    def extra_parameters(
        ...
        mapped_assumptions = tuple(
            assump.__class__(
                tuple(
                    self.forward_map(comb_class, gc, children)[0]
                    for gc in assump.gcs
                    if all(cell in self.forward_cell_map(comb_class) for cell in gc.pos)
                )
            ).avoiding(child.obstructions)
            for assump in comb_class.assumptions
        )
        return (
            {
    ...
    def backward_map(
        ...
        yield gc.apply_map(backmap.__getitem__)
    ...
    def forward_map(
        ...
        gc = obj.apply_map(forwardmap.__getitem__)
        return (gc,)
