...
# this looks unfinished, idk how important it is
class Interleaving(CartesianProduct[Tiling, GriddedChord]):
    def __init__(
        self,
        parent: Tiling,
    ...
    ) -> Iterator[Tuple[Parameters, Tuple[List[Optional[GriddedChord]], ...]]]:
...
class FactorWithInterleavingStrategy(FactorStrategy):
    ...
    def assumptions_to_add(self, comb_class: Tiling) -> Tuple[TrackingAssumption, ...]:
        ...
        return tuple(
            assump
            for assump in chain.from_iterable(
                self._assumptions_to_add(cells, cols, rows) for cells in self.partition
            )
            if assump not in comb_class.assumptions
        )
    ...
    def _assumptions_to_add(
        ...
        col_assumptions = [
            TrackingAssumption(
                [GriddedChord.point_chord(cell) for cell in cells if x == cell[0]]
            )
            for x in cols
        ]
        row_assumptions = [
            TrackingAssumption(
                [GriddedChord.point_chord(cell) for cell in cells if y == cell[1]]
            )
            for y in rows
        ]
        return tuple(assump for assump in chain(col_assumptions, row_assumptions) if assump.gps)
    ...
    def interleaving_parameters(self, comb_class: Tiling) -> List[Tuple[str, ...]]:
        ...
        for x in cols:
            ...
            res.append(
                tuple(
                    comb_class.get_assumption_parameter(assump)
    ...
    def backward_map(
        ...
        gps_to_combine = tuple(
            tiling.backward_map.map_gp(cast(GriddedChord, gp))
        ...
        for interleaved_gps_to_combine in all_gps_to_combine:
            ...
            new_pos = [(idx[0], val[0]) for idx, val in temp]
            new_patt = Chord.to_standard(val for _, val in temp)
            assert not GriddedChord(new_patt, new_pos).contradictory()
            yield GriddedChord(new_patt, new_pos)
