...
class Graph:
    ...
    def __init__(self, vertices: List[int], matrix: List[List[int]]=None):
...
class _RowColSeparationSingleApplication:
    ...
    def _basic_matrix(self, row):
        ...
        otherwise return it for the columns.
        """
        idx = 1 if row else 0
        m = []
    ...
    @staticmethod
    def _row_cell_order(ob: GriddedChord):
        """
    ...
    @staticmethod
    def _col_cell_order(ob: GriddedChord):
        """
    ...
    def _add_ineq(self, ineq: Cell, matrix: List[List[int]]):
        """
    ...
    def _complete_ineq_matrices(self):
        ...
        """
        if self._ineq_matrices is not None: # if _ineq_matrices has already been defined
    ...
    @staticmethod
    def _all_order(graph: Graph, only_max: bool=False):
        """
    ...
    @staticmethod
    def _maximal_order(graph: Graph):
    ...
    def map_assumptions(self, cell_map: Dict[Cell, Cell]):
        ...
        for assump in self._tiling.assumptions:
            gcs: List[GriddedChord] = []
            for gc in assump.gcs:
                mapped_gp = self._map_gridded_chord(cell_map, gc)
                if not mapped_gp.contradictory():
                    gcs.append(mapped_gp)
            yield assump.__class__(gcs)

    def map_linkages(self, cell_map: Dict[Cell, Cell]):
        """Map the linkages of a tiling according to the cell map."""
        for link in self._tiling.linkages:
            yield [cell_map[cell] for cell in link]
    ...
    def get_cell_map(self) -> Dict[Cell, Cell]:
        ...
        init_cell_map = sep_tiling.forward_map # need to implement
        res: Dict[Cell, Cell] = {}
        for cell in self._tiling.active_cells:
            mid_cell = sep_cell_map[cell]
            # If the cell is not in the init map it is an empty cell
            if init_cell_map.is_mappable_cell(mid_cell): # this is fine once rowcol map implemented
                final_cell = init_cell_map.map_cell(mid_cell) # this is fine once rowcol map implemented
                res[cell] = final_cell
        return res
